...
:::{.incremental}
- [**Plongements de mots**]{.orange} : [**vectorisation**]{.blue2}
- Plongements [**pré-entraînés**]{.orange} disponibles en open-source ;
- On apprend [**nos propres**]{.orange} plongements de mots ;
...
:::{.incremental}
- [**fastText**]{.blue2} : pas de prise en charge des variables catégorielles : concaténation avec le libellé ;
...
## Évaluation de la performance {auto-animate=true}
...
- Près de [**80%**]{.orange} des libellés issus du guichet unique sont correctement codififiés ;
...
plot_Accuracy_reprise <- function(data){
  plot<- ggplot(data , aes(x=Type, y=Accuracy, fill=Rate))+
    ggtitle("Taux de précision en fonction du taux de reprise manuelle effectuée")+
    geom_bar(stat = "identity", position = position_dodge())+
