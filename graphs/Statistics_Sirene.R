...
df_ <-df%>%
  ...
  select(LIA_NUM,DATE, APE_SICORE, LIB_SICORE, LIB_SICORE_NEW, EVT_SICORE, AUTO, NAT_SICORE, SURF)

##### Enhancing the libs #####
# Get all duplicated libs
...
# We create a new APE code when there is a different code for the same lib
HarmonizedAPE <- df_LibDuplicated%>%
  ...
  select(LIB_SICORE_NEW, APE_SICORE_NEW)


#df_new <- df_ %>%left_join(HarmonizedAPE)%>%
#  mutate(APE_SICORE_NEW = case_when(is.na(APE_SICORE_NEW) ~ APE_SICORE,
#                                    T ~ APE_SICORE_NEW)
#  )%>%
#  left_join(naf, by = "APE_SICORE_NEW")%>%
#  select(LIA_NUM,DATE, APE_SICORE, LIB_SICORE, LIB_SICORE_NEW, EVT_SICORE, AUTO, NAT_SICORE, SURF, NIV1, NIV2, NIV3, NIV4, LIB_NIV5)

##### Saving the new database with harmonized codes #####
