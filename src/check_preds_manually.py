...
tmp = Results.loc[:, ["LIA_NUM", "LIB_SICORE", "predictions_5_k1", "probabilities_k1", "predictions_5_k2", "probabilities_k2", "predictions_5_k3", "probabilities_k3", "predictions_5_k4", "probabilities_k4", "predictions_5_k5", "probabilities_k5"]]
tmp.columns = ["LIA_NUM", "LIB_SICORE", "CODE_APE_1", "PROBA_1", "CODE_APE_2", "PROBA_2", "CODE_APE_3", "PROBA_3", "CODE_APE_4", "PROBA_4", "CODE_APE_5", "PROBA_5"]

tmp.to_csv("../data/Preds_from_PY.csv")
# mc cp data/Preds_from_PY.csv minio/projet-ape/data/Preds_from_PY.csv


model.predict("vent produit fabriqu domicil decor principal AUTO_M NAT_SICORE_04 SURF_NaN EVT_SICORE_01P", k=5)
model.predict(list_libs[360], k=5)
model.predict("agit vendr particuli infus sachet vrac AUTO_C NAT_SICORE_10 SURF_NaN EVT_SICORE_01P", k=5)
