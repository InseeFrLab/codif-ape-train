...
def get_all_levels(df, df_naf, col):
    ...
    if df[col].dtype == "int64":
        inv_ape_niv5_mapping = get_inv_mapping("NAF2025")
...
def get_pred_nafs(predicted_class):
    """
...
def run_eval(
    ...
    naf_predictions = get_pred_nafs(predicted_class)
    all_level_preds = get_all_levels(naf_predictions, df_naf, "APE_NIV5_pred")
    all_level_ground_truth = get_all_levels(df, df_naf, Y)
