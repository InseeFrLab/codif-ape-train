...
class PreTokenizer(ABC):
    """
    Abstract base class for PreTokenizers.
    All PreTokenizers must implement the `clean_text_feature` method.
    """
    ...
    def pre_tokenize_splits(self, revision, df_train, df_val, df_test, **kwargs):
        def clean_df(df):
            df[self.TEXT_FEATURE] = self.clean_text_feature(
                df[self.TEXT_FEATURE].tolist(),
            )
    ...
    @abstractmethod
    def clean_text_feature(self, text: list[str], **kwargs) -> list:
        """
        Abstract method to pretokenize a given text.

        Args:
            text (str): The input text to be pretokenized.

        Returns:
            list: A list of pretokenized elements.
        """
        pass
    ...
    def clean_categorical_features(
        ...
        for surface_col in self.SURFACE_COLS:
            if surface_col in categorical_features:
                ...
                df = PreTokenizer.categorize_surface(df, surface_col)
