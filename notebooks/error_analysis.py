...
for idx in range(1, 6):
    categorical_features.append(
        mlflow.get_run(camembert_run_id).data.params.get(
            f"categorical_features_{idx}"
        )
    )
...
df_test_ls["categorical_inputs"] = df_test_ls[categorical_features].apply(lambda x: x.tolist(), axis=1)
