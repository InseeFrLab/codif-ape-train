...
dict_series = [
    parse_raw_input(raw_input, raw_fields, raw_regexes) for raw_input in df_raw_input.description
]
...
def extract_first_pred(predictions):
    regex = re.compile(
        r"proposé = ([^;]*) ;.*associée = ([^;]*)\].*proposé = ([^;]*) ;.*associée = ([^;]*)\]"
    )
...
dict_series = [
    parse_raw_input(info_input, info_fields, info_regexes) for info_input in df_info.description
]
...
dict_results = {
    f"pred_{pred+1}": [tmp[0][lib][pred][-5:] for lib in range(len(tmp[0]))] for pred in range(2)
} | {f"prob_{pred+1}": [tmp[1][lib][pred] for lib in range(len(tmp[0]))] for pred in range(2)}
...
def clean_lib(df, text_feature):
    ...
    df[text_feature] = df[text_feature].replace(to_replace=r"[^\w\s]", value=" ", regex=True)

    # On supprime tous les chiffres
    df[text_feature] = df[text_feature].replace(to_replace=r"[\d+]", value=" ", regex=True)

    # On supprime les longs mots sans sens
    df[text_feature] = df[text_feature].replace(to_replace=LongWord2remove, value="", regex=True)

    # On supprime les mots courts sans sens
    df[text_feature] = df[text_feature].replace(to_replace=Word2remove, value="", regex=True)

    # On supprime les mots d'une seule lettre
    df[text_feature] = df[text_feature].replace(to_replace=r"\b[a-z]{1}\b", value="", regex=True)
